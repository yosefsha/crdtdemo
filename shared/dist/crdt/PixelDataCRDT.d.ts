import {
  ...
  DocumentDeltaPacket,
  DocumentMergeResult,
} from "../crdt/database/CRDTDatabase";
export type RGBHEX = [number, number, number];
...
export declare class PixelDataCRDT {
  ...
  set(
    key: Key,
    color: RGBHEX | null
  ): import("./database/Collection").CollectionDelta<RGBHEX> | null;
  ...
  get values(): Record<string, RGBHEX>;
  toJSON(): {
    agentId: AgentId;
    replicaId: ReplicaId;
    documents: Record<
      import("./database/Document").DocumentId,
      ReturnType<import("./database/Document").Document<any>["toJSON"]>
    >;
  };
